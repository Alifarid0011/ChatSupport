<template>
  <div>
    <h1 class="text-center display-2 bg-yellow-100"> مهمان</h1>
    <div class="container">
            ...
            <div class="type_msg">
              <div class="input_msg_write">
                <input
                    @keydown="TypingEvent()"
                    v-model="newMessage"
                    @keyup.enter="SendMessage(guest_id,Support_id,newMessage)"
                    type="text"
                    class="write_msg"
...
export default {
  ...
  mounted() {
    this.isOpen(this.guest_id);
    window.Echo.channel('Chat').listen('SendMessage', (event) => {
      this.fetchMessages(this.guest_id,this.Support_id,'Guest');
    })
  },
  created() {
    this.showUser();
  ...
  methods: {
    ...
    async SendMessage(guest_id, support, Message) {
      let date = {
        guest_id: guest_id,
        support_id:  support,
        message: Message,
    ...
    async TypingEvent() {
      if (this.IsOpen[1] == null)
        await this.isOpen(this.guest_id);
